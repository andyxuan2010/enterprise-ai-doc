...
output "sql_server_fqdn" {
  ...
  value       = var.create_sql_database ? module.sqldb[0].server_fqdn : data.azurerm_mssql_server.landingzone_sql[0].fully_qualified_domain_name
}

output "sql_database_name" {
  description = "Name of the Azure SQL database."
  value       = var.create_sql_database ? module.sqldb[0].database_name : data.azurerm_mssql_database.landingzone_sql[0].name
}
