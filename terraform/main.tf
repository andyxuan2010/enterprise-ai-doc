...
module "functionapp" {
  ...
  source = "../../template//modules/functionapp"
  ...
  app_settings = merge(
    ...
    {
      SQL_SERVER_FQDN = var.create_sql_database ? module.sqldb[0].server_fqdn : data.azurerm_mssql_server.landingzone_sql[0].fully_qualified_domain_name
      SQL_DATABASE_NAME = (
        var.create_sql_database
        ? module.sqldb[0].database_name
        : data.azurerm_mssql_database.landingzone_sql[0].name
      )
    },
...
module "sqldb" {
  # create_sql_database=false reuses the existing SQL resources through data
  # sources in data.tf. Set it to true to create a new database and make the
  # sql_database_* / sql_firewall_rules / sql_administrator_* variables active.
